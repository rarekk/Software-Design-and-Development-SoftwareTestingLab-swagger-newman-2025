...
const collection = {
  ...
  item: [
    ...
    // ── Request 2: POST /api/bookings ────────────────────────
    {
      ...
      request: {
        ...
        body: {
          mode: 'raw',
          raw: JSON.stringify({
            fullname: 'นายวรปรัชญ์ บุญมี', email: '68030255@university.ac.th',
    ...
    }, // <-- แก้ไขปีกกาปิดให้ถูกต้องตรงนี้

    // ── Request 8: POST /api/login — Wrong Password ──────────
    {
      name: '8. POST /api/login — Wrong Password',
      event: [{ listen: 'test', script: { type: 'text/javascript', exec: [
        'pm.test("Returns 401 Unauthorized for wrong password", function() {',
        '  pm.response.to.have.status(401);',
        '});',
        'pm.test("Error message is correct", function() {',
        '  const d = pm.response.json();',
        '  pm.expect(d).to.have.property("error");',
        '  pm.expect(d.error).to.equal("ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง");',
        '});'
      ]}}],
      request: {
        method: 'POST',
